...
class TestSchemaValidation:
    ...
    def symbols_manifest(self) -> dict[str, Any]:
        ...
        manifest_path = (
            Path(__file__).parent.parent / "symbols" / "symbols_manifest.yaml"
        )
    ...
    def test_manifest_structure(self, symbols_manifest: dict[str, Any]):
        ...
        assert (
            "schema_version" in symbols_manifest
        ), "Manifest must have 'schema_version' key"
    ...
    def test_symbol_parameter_compliance(
        self, symbols_manifest: dict[str, Any], sample_json_output: dict[str, Any]
    ):
    ...
    def test_bbox_dimensions_match_manifest(
        ...
        for annotation in sample_json_output["annotations"]:
            ...
            assert (
                abs(bbox["w"] - symbol_schema["w_mm"]) < 0.001
            ), f"Width mismatch for {symbol_name}: {bbox['w']} != {symbol_schema['w_mm']}"
            assert (
                abs(bbox["h"] - symbol_schema["h_mm"]) < 0.001
...
def test_manifest_coverage():
    ...
    for category, expected_symbols in expected_categories.items():
        found_symbols = [
            ...
            if any(exp in name for exp in expected_symbols)
        ]
