...
class TestOverlayValidation:
    ...
    def test_iou_computation(self, sample_page_data: dict[str, Any]):
        ...
        for annotation in sample_page_data["annotations"]:
            expected_box = self._bbox_mm_to_pixels(
                annotation["bbox_mm"], sample_page_data["page_info"]
            )
            ...
            assert (
                iou > 0.8
            ), f"IoU too low for symbol {annotation['symbol_name']}: {iou}"
    ...
    def test_rotation_handling(self, sample_page_data: dict[str, Any]):
        ...
        # Find rotated symbol
        rotated_symbols = [
            ann for ann in sample_page_data["annotations"] if ann["rotation_deg"] != 0.0
        ]
        ...
        for symbol in rotated_symbols:
            ...
            ), "Rotated symbols should have positive dimensions"
    ...
    def _bbox_mm_to_pixels(
        self, bbox_mm: dict[str, float], page_info: dict[str, Any]
    ) -> dict[str, int]:
    ...
    def _detect_overlaps(
        ...
        for i in range(len(annotations)):
            for j in range(i + 1, len(annotations)):
                ...
                # Check for overlap
                if (
                    bbox1["x"] < bbox2["x"] + bbox2["w"]
                    and bbox1["x"] + bbox1["w"] > bbox2["x"]
                    and bbox1["y"] < bbox2["y"] + bbox2["h"]
                    and bbox1["y"] + bbox1["h"] > bbox2["y"]
                ):
                    overlaps.append((i, j))
