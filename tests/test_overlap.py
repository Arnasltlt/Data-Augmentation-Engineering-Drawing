...
from layoutlab.placer import (
    Symbol, PlacedSymbol, CollisionDetector, SymbolPlacer, 
    SheetDimensions, generate_page
)
...
class TestCollisionDetection(unittest.TestCase):
    ...
    def test_collision_with_minimum_spacing(self):
        ...
        self.assertFalse(CollisionDetector.check_collision(placed1, placed2, min_spacing=0.0))
        
        # With minimum spacing, should collide
        self.assertTrue(CollisionDetector.check_collision(placed1, placed2, min_spacing=2.0))
...
class TestSymbolPlacement(unittest.TestCase):
    ...
    def test_placement_within_sheet_bounds(self):
        ...
        for placed_symbol in placed_symbols:
            ...
            self.assertLessEqual(bbox[2], sheet_width - margin)  # x_max <= width - margin
            self.assertLessEqual(bbox[3], sheet_height - margin)  # y_max <= height - margin
        ...
        for i in range(len(placed_symbols)):
            for j in range(i + 1, len(placed_symbols)):
                ...
                self.assertFalse(collision, 
                    f"Collision detected between symbols {i} and {j}")
    ...
    def test_reproducible_placement(self):
        ...
        for p1, p2 in zip(placed1, placed2):
