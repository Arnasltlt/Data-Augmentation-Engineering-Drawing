...
def test_all_sheet_sizes():
    ...
    for sheet_size in sheet_sizes:
        ...
        try:
            ...
            print(
                f"  ✓ Success: {len(result['annotations'])} symbols in {duration_ms:.1f}ms"
            )

        except Exception as e:
            results[sheet_size] = {"success": False, "error": str(e)}
            print(f"  ✗ Failed: {e}")
    ...
    for sheet_size, result in results.items():
        if result["success"]:
            print(
                f"  {sheet_size}: ✓ {result['symbols_placed']} symbols, "
