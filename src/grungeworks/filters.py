...
class SkewPerspectiveFilter(BaseFilter):
    ...
    def apply(self, img: Image.Image) -> Image.Image:
        ...
        perspective_factor = np.random.uniform(-self.max_perspective, self.max_perspective)
        ...
        skewed = cv2.warpAffine(img_array, skew_matrix, (w, h), 
        ...
        if abs(perspective_factor) > 0.001:
            ...
            warped = cv2.warpPerspective(skewed, perspective_matrix, (w, h),
                                       borderMode=cv2.BORDER_CONSTANT,
                                       borderValue=(255, 255, 255))
        else:
            warped = skewed
