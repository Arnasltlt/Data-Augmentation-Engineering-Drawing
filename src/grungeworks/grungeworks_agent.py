...
class GrungeWorksAgent:
    ...
    def convert_pdf_to_png(self, pdf_path: str, output_path: str, dpi: int = 300) -> bool:
        ...
        try:
            ...
            try:
                ...
                if images:
                    ...
                    self.logger.info(f"Converted PDF to PNG (pdf2image): {pdf_path} -> {output_path}")
                    return True
                    
            except ImportError:
                self.logger.error("Neither PyMuPDF nor pdf2image available for PDF conversion")
                return False
    ...
    def process_page(self, pdf_path: str, json_path: str, noise_level: int = 1) -> bool:
        ...
        try:
            ...
            if not self._verify_coordinate_alignment(json_path, png_path):
                self.logger.warning(f"Coordinate alignment verification failed for {png_path}")
                
            self.logger.info(f"Processed page with noise level {noise_level}: {png_path}")
            return True
    ...
    def _apply_noise_pipeline(self, img: Image.Image, noise_level: int) -> Image.Image:
        ...
        Returns:
            Processed PIL Image
        """
        from .filters import GaussianBlurFilter, JPEGArtifactFilter, SkewPerspectiveFilter
        
        # Define noise level presets
        presets = {
            0: [],  # No noise
            1: [
                GaussianBlurFilter(sigma=0.3),
                JPEGArtifactFilter(quality=85)
            ],
            2: [
        ...
        for i, filter_obj in enumerate(filters):
            ...
            if self.debug:
                debug_path = Path(str(img.filename).replace('.png', f'_debug_{i}_{filter_obj.__class__.__name__}.png'))
        ...
        try:
            ...
            try:
                ...
                if width < 100 or height < 100:
                    self.logger.warning(f"PNG dimensions seem too small: {width}x{height}")
                    return False
                ...
                for i, symbol in enumerate(symbols):
                    ...
                    if x_px < 0 or x_px >= width or y_px < 0 or y_px >= height:
                        self.logger.warning(f"Symbol {i} coordinates out of bounds: ({x_px:.1f}, {y_px:.1f})")
