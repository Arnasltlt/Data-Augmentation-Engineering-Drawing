...
class CompletionScoreCalculator:
    ...
    def check_symbol_coverage(self) -> int:
        ...
        try:
            ...
            self.score_breakdown["symbol_coverage"] = (
                f"{actual_symbols}/{target_symbols} symbols, parity: {parity_score > 0}"
            )
    ...
    def check_layout_engine(self) -> int:
        ...
        try:
            ...
            missing_files = [
                f for f in required_files if not (layoutlab_path / f).exists()
            ]

            if missing_files:
                self.score_breakdown["layout_engine"] = (
        ...
        try:
            ...
            try:
                ...
                if result.returncode == 0:
                    self.score_breakdown["end_to_end"] = (
                        "Generator help runs successfully"
                    )
                    return 20
                else:
                    self.score_breakdown["end_to_end"] = (
                        f"Generator help failed: {result.stderr}"
                    )
                    return 10
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    self.score_breakdown["test_coverage"] = (
                        f"{len(test_files)} test files, tests pass"
                    )
                    return score
                else:
                    self.score_breakdown["test_coverage"] = (
    ...
    def check_ci_status(self) -> int:
        ...
        try:
            ...
            self.score_breakdown["ci_status"] = (
                f"{len(workflow_files)} workflow files configured"
            )
...
def main():
    ...
    if args.format == "json":
        ...
        print(
            f"🔍 Project Completion Score: {result['total_score']}/{result['max_score']}"
        )
