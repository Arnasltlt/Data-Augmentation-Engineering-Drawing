...
def generate_pages(args):
    ...
    try:
        ...
        print("Make sure LayoutLab and GrungeWorks agents are implemented", file=sys.stderr)
        sys.exit(1)
...
def generate_pages_parallel(args, output_dir: Path):
    ...
    try:
        ...
        @ray.remote
        def generate_single_page(page_num: int, sheet_size: str, noise_level: int, output_dir_str: str):
        ...
        print(f"Successfully generated {args.num_pages} page(s) in {elapsed_time:.2f}s ({pages_per_second:.1f} pages/s)")
